...
export class TeamsComponent implements OnInit {
  ...
  constructor(private serviceModel: NgbModal, private httpRequestService:HttpRequestService) { 
    
    this.teamList = new Array<Team>();

    if (this.tempMaps.length > 0)
      this.selectedMission = this.tempMaps[0];

    this.tempData.Agents = new Array<Agent>(5);
    for (let i = 0; i < 3; i++)
    {
      this.tempData.Agents[i] = this.tempAgentData;
    }

    this.teamList.push(this.tempData);
    this.teamList.push(this.tempData);
    this.teamList.push(this.tempData);
    this.teamList.push(this.tempData);
  ...
  ngOnInit(): void {
    /*
    this.httpRequestService.getPlayerTeams().subscribe(result => {
      ...
      {
        let team!: Team;
        console.log(teams[i].agents);
        for (let j = 0; j < teams[i].agents.length; j++)
        {
          this.httpRequestService.getAgent(teams[i].agents[j].uuid).subscribe(agentResult => {
            var data = JSON.parse(agentResult.data);
            team.Agents.push(data.data);
          })
        }
  ...
  changeSelectedMission(selectedMission:any)
  {
    for (let mission of this.tempMaps)
    {
      if (mission.uuid == selectedMission.target.value)
      {
  ...
  openModal(team:Team, content:any)
  {
